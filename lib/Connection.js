...
Connection.prototype.init = function() {
	...
	this.socket.on('open', function() {
		self.loadSchema().then(function(schema) {
			...
			self.socket.on('message', function(message, isBinary) {
				message = isBinary ? message : message.toString();
				try {
					self.schema.handleResponse(message);
				} catch(err) {
					err.message = 'Failed to handle response: ' + err.message + ', Data: ' + (Buffer.isBuffer(message) ? message.toString('hex') : message);
					self.emit('error', err);
				}
			});
